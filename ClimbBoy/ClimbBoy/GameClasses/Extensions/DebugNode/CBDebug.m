...
@implementation CBDebug

DEFINE_SINGLETON_FOR_CLASS(CBDebug)
...
+ (void) drawLineStart:(CGPoint)start end:(CGPoint)end {
    [[CBDebug sharedCBDebug] drawLineStart:start end:end];
}


+ (void) drawLineStart:(CGPoint)start end:(CGPoint)end color:(SKColor *)color {
    [[CBDebug sharedCBDebug] drawLineStart:start end:end color:color];
}
